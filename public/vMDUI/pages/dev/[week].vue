      ...
      <table>
        ...
        <tbody>
          <tr v-for="{ device, name, rom } in data.roms">
            ...
            <td v-else><a :href="('https://bkt-sgp-miui-ota-update-alisgp.oss-ap-southeast-1.aliyuncs.com/' + rom.os + '/' + rom.fastboot)"><span v-if="$device.isDesktopOrTablet">{{ rom.fastboot }}</span><span v-else>{{ $t('fastboot') }}</span></a></td>
          </tr>
        </tbody>
      </table>
    </div>
