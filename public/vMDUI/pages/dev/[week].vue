      ...
      <table>
        ...
        <tbody>
          <tr v-for="{ device, name, rom } in data.roms">
            ...
            <td v-else><a :href="('https://cdn-ota.azureedge.net/' + rom.os + '/' + rom.fastboot)"><span v-if="$device.isDesktopOrTablet">{{ rom.fastboot }}</span><span v-else>{{ $t('fastboot') }}</span></a></td>
          </tr>
        </tbody>
      </table>
    </div>
