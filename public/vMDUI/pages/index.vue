<template>
  ...
  <mdui-card style="width: 99vw;padding-bottom: 20px;align-items: center;margin-top:1vw;margin-right:1vw">
    ...
    <h3 style="padding-left:10px;">{{ $t('supported') }}</h3><span v-for="({ code, name },index) in device.mi" style="padding-left:10px;text-indent: 20px;">
      <span v-if="index<device.mi.length-1"><NuxtLink :to="('/' + locale + '/devices/' + code)" style="text-indent: 20px;" class="HyperBlue">{{ name[locale] }}</NuxtLink>;</span>
      <span v-else><NuxtLink :to="('/' + locale + '/devices/' + code)" class="HyperBlue" style="text-indent: 20px;">{{ name[locale] }}</NuxtLink></span>
    </span>
  </mdui-card>
  <mdui-card style="width: 99vw;padding-bottom: 20px;align-items: center;margin-top:1vw;margin-right:1vw">
    ...
    <span v-for="({ device, name },index) in latest.roms" style="padding-left:10px;" v-show="latest.show == 'yes'">
      <span v-if="index<latest.roms.length-1"><NuxtLink :to="('/' + locale + '/devices/' + device)" class="HyperBlue" style="text-indent: 20px;">{{ name[locale] }}</NuxtLink>;</span>
      <span v-else><NuxtLink :to="('/' + locale + '/devices/' + device)" class="HyperBlue" style="text-indent: 20px;">{{ name[locale] }}</NuxtLink></span>
    </span>
  </mdui-card>
  <mdui-card style="width: 99vw;padding-bottom: 20px;align-items: center;margin-top:1vw;margin-right:1vw">
    ...
    <ol v-for="({ code, name, rom },index) in home.recent.roms" style="padding-left:10px;list-style-type:none;">
      <li v-if="index<home.recent.roms.length-1"><NuxtLink :to="('/' + locale + '/devices/' + code)" class="HyperBlue" style="text-indent: 20px;">{{ name[locale] }}</NuxtLink> : {{ rom }};</li>
      <li v-else><NuxtLink :to="('/' + locale + '/devices/' + code)" class="HyperBlue" style="text-indent: 20px;">{{ name[locale] }}</NuxtLink> : {{ rom }}</li>
    </ol>
  </mdui-card>
