	...
	<v-app>
		...
		<main id="main-content" tabindex="-1">
			...
			<v-expansion-panels variant="accordion" v-model="panel">
				<v-expansion-panel v-for="branche in data['branches']" v-show="branche['show'] == '1'" :key="branche['branchCode']">
					...
					<v-expansion-panel-text>
						<v-table>
							...
							<thead>
								<tr>
									<th class="text-left" scope="col" v-for="menu in branche['table']" :key="menu">{{ $t(menu) }}</th>
									<!-- <th class="text-left">{{ $t('detail') }}</th> -->
								</tr>

							</thead>
							<tbody>
								<tr v-for="(rom, ver) in branche['roms']" :key="ver">
									<td v-for="i in branche['table'].length" :key="i">
										...
										<span v-else-if="branche['table'][i - 1] == 'originrec' || branche['table'][i - 1] == 'originfb'">
											<a :href="(rom[branche['table'][i - 1]])" :aria-label="$t('getitdown') + ': ' + rom[branche['table'][i - 1]]">
												<span v-if="$device.isDesktopOrTablet">{{ rom[branche['table'][i - 1]] }}</span>
												<span v-else>{{ $t('getitdown') }}</span>
											</a>
										</span>
										<span v-else>
											<a :href="('https://bkt-sgp-miui-ota-update-alisgp.oss-ap-southeast-1.aliyuncs.com/' + ver + '/' + rom[branche['table'][i - 1]])" :aria-label="$t('getitdown') + ': ' + rom[branche['table'][i - 1]]">
												<span v-if="$device.isDesktopOrTablet">{{ rom[branche['table'][i - 1]] }}</span>
												<span v-else>{{ $t('getitdown') }}</span>
											</a>
										</span>
...
// 使用 useAsyncData 确保数据在服务端和客户端保持一致
const { data } = await useAsyncData('device-' + route.params.codename, () =>
	$fetch(url)
);
</script>